...
async function fetchAllExercises(): Promise<CachedExercise[]> {
  ...
  while (true) {
    const { data, error } = await supabase
      .from("v_exercises_compact") // use the view above; fallback to exercises with a small select if needed
...
export default function PlanInfo() {
  ...
  return (
    <View style={s.page}>
      ...
      <View style={{ flexDirection: "row", gap: 8 }}>
        {[1, 2, 3, 4, 5, 6].map((n) => (
          <Pressable
            ...
            style={[s.chip, workoutsPerWeek === n && s.chipActive]}
          >
            <Text
              style={{
                fontWeight: "700",
                color: workoutsPerWeek === n ? "#fff" : "#111",
              }}
            >
              {n}
            </Text>
      ...
      <Pressable style={s.primary} onPress={proceed}>
        ...
        <View style={s.modalScrim}>
          <View style={s.modalCard}>
            ...
            <View style={{ flexDirection: "row", gap: 8, marginTop: 12 }}>
              <Pressable style={s.btn} onPress={() => setShow(false)}>
