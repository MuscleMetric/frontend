...
export type WorkoutDraftAction =
  ...
  | { type: "draft/hydrate"; payload: { draft: WorkoutDraft; nowIso: string } } // ✅ NEW
  ...
  | {
      ...
      payload: { exerciseKey: string; nowIso: string };
    }
  | {
      type: "draft/reorderExercises";
      payload: { from: number; to: number; nowIso: string };
    }
  | {
  ...
  | {
      ...
      payload: { exerciseKey: string; note: string; nowIso: string };
    }
  | {
  ...
  | {
      ...
      payload: { group: string; nowIso: string };
    }
  | {
      type: "draft/markSaved";
      payload: { snapshotHash: string; nowIso: string };
    }
