...
import { Card, Button, Pill, WorkoutCover, ListRow, Icon, ModalSheet } from "@/ui";
...
export default function WorkoutOverviewScreen() {
  ...
  const ex = React.useMemo(() => {
    if (!bootstrap) return [];
    return bootstrap.exercises.slice().sort((a, b) => (a.orderIndex ?? 0) - (b.orderIndex ?? 0));
  }, [bootstrap]);

  const activeExercise = React.useMemo(() => {
    if (!bootstrap || !activeExerciseId) return null;
    return bootstrap.exercises.find((x) => x.exerciseId === activeExerciseId) ?? null;
  ...
  if (error || !bootstrap) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.bg, padding: layout.space.lg }}>
        <Text style={{ color: colors.danger, fontFamily: typography.fontFamily.semibold }}>
      ...
      >
        {!activeExercise ? null : (
          <View style={{ gap: layout.space.md }}>
            {/* quick meta */}
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: layout.space.sm }}>
              {activeExercise.equipment ? <Pill label={activeExercise.equipment} tone="neutral" /> : null}
              {activeExercise.level ? <Pill label={activeExercise.level} tone="neutral" /> : null}
              {activeExercise.prescription?.targetSets ? (
                <Pill label={`${activeExercise.prescription.targetSets} sets`} tone="primary" />
              ) : null}
              {activeExercise.prescription?.targetReps ? (
                <Pill label={`${activeExercise.prescription.targetReps} reps`} tone="primary" />
              ) : null}
            </View>

            {/* instructions */}
            {activeExercise.instructions ? (
              <Text style={{ color: colors.textMuted, lineHeight: typography.lineHeight.body }}>
                {activeExercise.instructions}
              </Text>
            ) : null}
              ...
              <View style={{ gap: layout.space.sm }}>
                ...
                <Text style={{ color: colors.textMuted }}>
                  {activeExercise.lastSession.completedAt
                    ? `Completed: ${formatLastDone(activeExercise.lastSession.completedAt)}`
                    : "No history yet"}
                </Text>

                {activeExercise.lastSession.sets?.length ? (
                  <View style={{ gap: 8 }}>
                    {activeExercise.lastSession.sets.slice(0, 6).map((s) => (
                      <View
                      ...
                      >
                        <Text style={{ color: colors.text }}>{`Set ${s.setNumber}`}</Text>
                        <Text style={{ color: colors.textMuted }}>
